...
def home(request):
    ...
    # Search mode — use cached context + override search fields
    if image or query or category_id:
        lang_code = request.LANGUAGE_CODE
        cache_key = f'home_context_{lang_code}'
    ...
    # Normal page load — use cached context
    lang_code = request.LANGUAGE_CODE
    cache_key = f'home_context_{lang_code}'
    context = cache.get(cache_key)
    if context is None:
    ...
    if user.is_authenticated:
        ...
        if interest_cat_ids:
            rec_ids = list(base_qs.filter(category_id__in=interest_cat_ids).values_list('id', flat=True))
            random.Random(datetime.date.today().toordinal()).shuffle(rec_ids)
            recommendations = list(base_qs.filter(id__in=rec_ids[:8]))
        if not recommendations:
            recommendations = list(base_qs.filter(is_featured=True)[:4])
    else:
        recommendations = list(base_qs.filter(is_featured=True)[:4])
